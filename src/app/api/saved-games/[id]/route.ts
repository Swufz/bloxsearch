...
export async function DELETE(
  ...
  query = resolved
    ? query.eq("game_id", resolved.databaseId)
    : uuidLike
      ? query.or(`id.eq.${id},game_id.eq.${id}`)
      : query.eq("id", "00000000-0000-0000-0000-000000000000");
  const { error } = await query;
  if (error) {
    console.error("[saved-games] Failed to remove saved game", {
      userId: auth.user.id,
      id,
      resolvedDatabaseId: resolved?.databaseId,
      error,
    });
    return NextResponse.json(
