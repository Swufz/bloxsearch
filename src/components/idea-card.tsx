...
export function IdeaCard({ idea, showSave = true, demoMode = false }: { idea: GeneratedIdea; showSave?: boolean; demoMode?: boolean }) {
  return (
    <div className="card p-5">
      ...
      {showSave && <button disabled={demoMode} title={demoMode ? "Saving requires login. Demo browsing is available." : "Save idea"} className="mt-4 inline-flex items-center gap-2 rounded-lg border border-slate-700 px-3 py-2 text-xs font-medium text-slate-200 hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-40"><BookmarkPlus size={14} />Save idea</button>}
    </div>
  );
}
