...
const links = [
  ...
  { href: "/ideas/top", label: "Top Ideas", icon: Sparkles },
...
export function AppShell({
  ...
  return (
    <div className="min-h-screen bg-[#0B0F19]">
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-40 w-64 border-r border-slate-800 bg-[#0B0F19] p-5 transition-transform lg:translate-x-0",
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-slate-800 bg-[#0B0F19]/90 px-4 backdrop-blur md:px-8">
          ...
          <div className="flex items-center gap-3">
            <span className="hidden text-sm text-slate-400 sm:inline">
              {userEmail ? "Signed in" : "Demo workspace"}
            </span>
            <AuthActions userEmail={userEmail} />
            <span className="grid h-8 w-8 place-items-center rounded-full bg-gradient-to-br from-sky-400 to-emerald-400 text-xs font-bold text-slate-950">
              BS
            </span>
          </div>
        </header>
